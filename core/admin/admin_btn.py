...
class GwAdminButton:
    ...
    def _init_show_database(self):
        ...
        self.cmb_geon_connection = self.dlg_readsql.findChild(QComboBox, 'cmb_geon_connection')
        self.cmb_geon_project_list = self.dlg_readsql.findChild(QComboBox, 'cmb_geon_project_list')
        self.btn_geon_open_projet = self.dlg_readsql.findChild(QComboBox, 'btn_geon_open_projet')
        ...
        self.geon_test_info = self.dlg_readsql.findChild(QTextEdit, 'text_geon_test')

        self._manage_utils()
        
        #Geon connection list
        tools_qt.fill_combo_values(self.cmb_geon_connection, self.list_connections, 1)
    ...
    def _set_signals(self):
        ...
        self.dlg_readsql.btn_geon_open_projet.clicked.connect(partial(self._geon_open_project)) 
    ...
    def _set_open_project(self):
        ...
        self.geon_projects = []
        try:
            url = f"http://{self.geon_dict['host']}:8088/api/v1/geon/projects"
            payload = ""
            headers = {
            ...
            tools_qt.set_widget_text(self.dlg_readsql, 'lbl_geon_status', 'The connection is wrong or there is no "project" schema in your database')
            elem = ['','']   
            self.dlg_readsql.btn_geon_open_projet.setEnabled(False)     
            self.geon_projects.append(elem)
        ...
        big_text += f'PostGis version: {self.postgis_version}\n \n'
        big_text += f"Username\t: {self.geon_dict['username']}\n"
        big_text += f"Password\t: {self.geon_dict['password']}\n"
        big_text += f"Database\t: {self.geon_dict['database']}\n"
        big_text += f"Host\t: {self.geon_dict['host']}\n"
        big_text += f"Port\t: {self.geon_dict['port']}\n"
        big_text += f"Service\t: {self.geon_dict['service']}\n"
        big_text += f"SSLMode\t: {self.geon_dict['sslmode']}\n"
        big_text += f"Auth CFG\t: {self.geon_dict['authcfg']}\n"
        
        tools_qt.fill_combo_values(self.dlg_readsql.cmb_geon_project_list, self.geon_projects, 1)
        
        self.geon_test_info.setReadOnly(True)
        self.geon_test_info.setText(big_text)
    
    def _choose_geon_project(self):
        """"""        
        self.geon_project_name = tools_qt.get_text(self.dlg_readsql, self.dlg_readsql.cmb_geon_project_list)
    
    def _geon_open_project(self):
        """Open project from postgresql schema 'project'"""    
